    ...
    public class TransferHandling {
        ...
        public static Integer getBillProviderIdFromName(String providerName) {
            String query = "SELECT bill_provider_id FROM bill_provider_supported WHERE bill_provider_name = ?";

            // Use try-with-resources to automatically manage database objects
            try (Connection conn = DatabaseConnection.getConnection();
                 PreparedStatement stmt = conn.prepareStatement(query)) {

                // Bind the input parameter safely to prevent SQL injection
                stmt.setString(1, providerName);

                try (ResultSet rs = stmt.executeQuery()) {
                    if (rs.next()) {
                        // Return the ID if a matching provider is found
                        return rs.getInt("bill_provider_id");
                    }
                }

            } catch (Exception e) {
                System.err.println("Error fetching bill provider ID for name: " + providerName);
                e.printStackTrace();
            }

            // Return null if no provider was found or if an exception occurred
            return null;
        }

        public static List<String> getSupportedBillProvider(String billType) {
            // 1. Initialize the result list up front so we can always return it (even if empty)
            List<String> result = new ArrayList<>();

            String typeQuery = "SELECT bill_id FROM bill_type_supported WHERE bill_type_name = ?";
            String providerQuery = "SELECT bill_provider_name FROM bill_provider_supported WHERE bill_id = ?";

            // 2. Using nested try-with-resources to automatically close connections and statements
            try (Connection conn = DatabaseConnection.getConnection();
                 PreparedStatement stmt1 = conn.prepareStatement(typeQuery)) {

                // Bind the billType parameter (assuming your DB column matches the string input)
                stmt1.setString(1, billType);

                Integer billId = null;
                try (ResultSet rs1 = stmt1.executeQuery()) {
                    if (rs1.next()) {
                        billId = rs1.getInt("bill_id");
                    }
                }

                // 3. If we found a matching bill_id, go fetch the providers
                if (billId != null) {
                    try (PreparedStatement stmt2 = conn.prepareStatement(providerQuery)) {
                        stmt2.setInt(1, billId); // Bind the retrieved billId

                        try (ResultSet rs2 = stmt2.executeQuery()) {
                            while (rs2.next()) {
                                result.add(rs2.getString("bill_provider_name"));
                            }
                        }
                    }
                }
                ...
                System.err.println("Something went wrong while fetching bill providers");
                e.printStackTrace();
            }

            // 4. Always return the list (it will be empty if an error occurred or no providers were found)
            return result;
        }
        /**
